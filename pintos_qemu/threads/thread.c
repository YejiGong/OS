...
//for mlfq, 4 level of ready list (lowest:3 highest:0)
...
/* Scheduling. */
#define TIME_SLICE 7            /* # of timer ticks to give each thread. */
...
thread_tick (void) 
{
  ...
  for(int i=t->priority+1; i<PRI_MIN+1; i++){
    if (list_empty(&ready_list[i])){
      ...
      for(tmp = list_begin (&ready_list[i]); tmp != list_end (&ready_list[i]); tmp = list_next (tmp)){
        ...
        if(tmp_thread->age == 20){
          ...
          prev = list_prev(tmp);
          list_remove(tmp);
          list_push_back(&ready_list[i-1], tmp);
          tmp = prev;
        }
        }
      }
  ...
  /* Enforce preemption. */
  if (++thread_ticks >= TIME_SLICE-(PRI_MIN-t->priority)){
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  ASSERT (t != NULL);
  ASSERT (PRI_MAX <= priority && priority <= PRI_MIN);
  ASSERT (name != NULL);
